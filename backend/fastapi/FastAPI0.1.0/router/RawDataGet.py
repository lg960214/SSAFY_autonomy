...
def postSensorData(productionName,sensorData: SensorRawData):
    try :
        ...
        p = influxdb_client.Point("SensorData").tag("Name", productionName).field("MM",sensorData.MM).field("DD",sensorData.DD).field("HH",sensorData.HH).field("Min",sensorData.Min).field("Sec",sensorData.Sec).field("Day",sensorData.Day).field("Illuminance",sensorData.Illuminance).field("Manual",sensorData.Manual).field("Brightness",sensorData.Brightness).field("Movement",sensorData.Movement).field("On",sensorData.On)
