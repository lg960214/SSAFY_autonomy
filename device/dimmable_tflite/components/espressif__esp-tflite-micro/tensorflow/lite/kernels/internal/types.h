...
template <int N>
int ArraySize_tf(const Dims<N>& array, int index) {
...
int MatchingArraySize(const ArrayType1& array1, int index1,
  ...
  TFLITE_DCHECK_EQ(ArraySize_tf(array1, index1), ArraySize_tf(array2, index2));
  return ArraySize_tf(array1, index1);
}
...
inline int MatchingFlatSize(const Dims<N>& dims, const Dims<N>& check_dims_0) {
  for (int i = 0; i < N; ++i) {
    TFLITE_DCHECK_EQ(ArraySize_tf(dims, i), ArraySize_tf(check_dims_0, i));
  }
  return FlatSize(dims);
}
